...
class ZGBroadcast(BASE):
    ...
    def __repr__(self):
        return "<ZG Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return (
            isinstance(other, ZGBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


ZGBroadcast.__table__.create(checkfirst=True)

ZGSBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with ZGSBROADCAST_INSERTION_LOCK:
        broadcast_group = ZGBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with ZGSBROADCAST_INSERTION_LOCK:
        if broadcast_group := SESSION.query(ZGBroadcast).get(
...
def is_in_broadcastlist(keywoard, group_id):
    with ZGSBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(ZGBroadcast).get((keywoard, str(group_id)))
...
def del_keyword_broadcastlist(keywoard):
    with ZGSBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(ZGBroadcast.keywoard)
            .filter(ZGBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(ZGBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(ZGBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(ZGBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(ZGBroadcast).all()
        for x in all_groups:
