...
class ZG_GlobalCollection(BASE):
    ...
    def __repr__(self):
        return "<ZG Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return (
            isinstance(other, ZG_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


ZG_GlobalCollection.__table__.create(checkfirst=True)

ZGS_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with ZGS_GLOBALCOLLECTION:
        keyword_items = ZG_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with ZGS_GLOBALCOLLECTION:
        if keyword_items := SESSION.query(ZG_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with ZGS_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(ZG_GlobalCollection.keywoard)
            .filter(ZG_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(ZG_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(ZG_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(ZG_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(ZG_GlobalCollection).all()
        for x in all_groups:
