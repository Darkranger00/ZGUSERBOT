...
class ZGGloballist(BASE):
    ...
    def __repr__(self):
        return "<ZG global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return (
            isinstance(other, ZGGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


ZGGloballist.__table__.create(checkfirst=True)

ZGSGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with ZGSGLOBALLIST_INSERTION_LOCK:
        broadcast_group = ZGGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with ZGSGLOBALLIST_INSERTION_LOCK:
        if broadcast_group := SESSION.query(ZGGloballist).get(
...
def is_in_list(keywoard, group_id):
    with ZGSGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(ZGGloballist).get((keywoard, str(group_id)))
...
def del_keyword_list(keywoard):
    with ZGSGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(ZGGloballist.keywoard)
            .filter(ZGGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(ZGGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(ZGGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(ZGGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(ZGGloballist).all()
        for x in all_groups:
