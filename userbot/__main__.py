...
from .core.session import ZGBOT, tgbot
from .utils import (
...
async def zazbhai():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", ZG_NAME)
            await asyncio.sleep(1)
...
@ZGBOT.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
@ZGBOT.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"starts")))
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [userbot](https://t.me/ZG_PRO)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/ZG_PRO"),
                    Button.url(" Updates ", "https://t.me/ZG_PRO"),
                ],
                [
...
@ZGBOT.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
async def res_ttart(event):
    if event.query.user_id == bot.uid:
        await event.answer("Restarting Please Wait 4 min... ", cache_time=0, alert=True)
        if BOTLOG:
            ZGBOT = await event.client.send_message(
                BOTLOG_CHATID, "# RESTART \n" "Bot Restarted"
            )
        try:
            await ZGBOT.disconnect()
...
@ZGBOT.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@ZGBOT.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using ZGBOT", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using ZGBOT", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using ZGBOT", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using ZGCEOSBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `ZGBOT Is best` you can login now\n\nThanks For using ZGBOT.",
                buttons=keyboard,
            )
        else:
...
try:
    ...
    ZGBOT.loop.run_until_complete(setup_bot())
...
async def startup_process():
    ...
    print(
        f"Congratulation, now type {cmdhr}alive to see message if ZGBOT is live\
        \nIf you need assistance, head to https://t.me/zazbhai"
    )
    print("============================================================")
    await verifyLoggerGroup()
...
async def externalrepo():
    ...
    #if Config.ZGABUSE:
       # await install_externalrepo(
           # Config.ZGABUSE_REPO, Config.ZGABUSE_REPOBRANCH, "badcatext"
        #)
    if Config.VCMODE:
        await install_externalrepo(
            "https://github.com/Zazbhai/ZG-VC", "test", "catvc"
        )


ZGBOT.loop.run_until_complete(startup_process())

ZGBOT.loop.run_until_complete(externalrepo())

if len(sys.argv) in {1, 3, 4}:
    with contextlib.suppress(ConnectionError):
        ZGBOT.run_until_disconnected()
else:
    ZGBOT.disconnect()
