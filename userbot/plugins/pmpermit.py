...
You have {warns}/{totalwarns} warns until you get blocked by the ZGBOT.
    ...
    try:
        if gvarstatus("pmmenu") is None:
            ...
            if PM_PIC := gvarstatus("pmpermit_pic"):
                ZGS = list(PM_PIC.split())
                PIC = list(ZGS)
                ZGS_IMG = random.choice(PIC)
            else:
                ZGS_IMG = None
            if ZGS_IMG is not None:
...
@ZGBOT.zazbhai_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@ZGBOT.zazbhai_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
