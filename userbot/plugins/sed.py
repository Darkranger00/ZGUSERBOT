...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await ZGBOT(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await ZGBOT(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@ZGBOT.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@ZGBOT.on(events.MessageEdited)
