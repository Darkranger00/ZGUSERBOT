...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, ZGBOT
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"do `{cmdhd}update deploy` to update the ZGBOT"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/zazbhaianand2/ZG-V2"
    os.chdir("/app")
    try:
        txt = (
            "`Oops.. Updater cannot continue due to "
    ...
    if ENV:
        ...
        if cmd == "good":
            if ZGABUSE:
                ...
                return
            await edit_delete(event, "`You already using GoodZG`", 6)
        else:
            if ZGABUSE:
                return await edit_delete(event, "`You already using BadZG`", 6)
            await edit_or_reply(
        ...
        if cmd == "good":
            ...
            await edit_delete(event, "`You already using GoodZG`")
        elif cmd == "bad":
            if match and ZGABUSE:
                return await edit_or_reply(event, "`You already using BadZG`")
