...
async def switch_branch():
    with open(config, "r") as f:
        configs = f.read()
    BRANCH = "master"
    REPO = "https://github.com/zazbhaianand2/ZG-V2"
    ZGABUSE = EXTERNAL = False
    for match in re.finditer(
        r"(?:(UPSTREAM_REPO|UPSTREAM_REPO_BRANCH|EXTERNAL_REPO|ZGABUSE)(?:[ = \"\']+(.*[^\"\'\n])))",
        configs,
    ):
        ...
        ZGABUSE = (
            True
            if match.group(1) == "ZGABUSE" and match.group(2).lower() != "false"
            else ZGABUSE
        )
    if REPO:
        await _catutils.runcmd(f"git clone -b {BRANCH} {REPO} TempZG")
        file_list = os.listdir("TempZG")
        for file in file_list:
            await _catutils.runcmd(f"rm -rf {file}")
            await _catutils.runcmd(f"mv ./TempZG/{file} ./")
        await _catutils.runcmd("pip3 install --no-cache-dir -r requirements.txt")
        await _catutils.runcmd("rm -rf TempZG")
    if not ZGABUSE and os.path.exists("badcatext"):
        ...
        if variable not in var_checker:
            if variable == "EXTERNAL_REPO":
                if bool(value and (value.lower() != "false")) and not url(value):
                    value = "https://github.com/zazbhaianand2/tha_plugins"
                else:
                    return await edit_or_reply(
                        cat,
