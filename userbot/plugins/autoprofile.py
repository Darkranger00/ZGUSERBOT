...
from . import BOTLOG, BOTLOG_CHATID, _catutils, ZGBOT, edit_delete, logging
...
async def autopicloop():
    ...
    if AUTOPICSTART and DEFAULT_PIC is None:
        if BOTLOG:
            return await ZGBOT.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await ZGBOT.upload_file(autophoto_path)
        try:
            await ZGBOT(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await ZGBOT.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await ZGBOT(
                    functions.photos.DeletePhotosRequest(
                        await ZGBOT.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await ZGBOT(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await ZGBOT(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await ZGBOT(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
ZGBOT.loop.create_task(autopfp_start())
ZGBOT.loop.create_task(autopicloop())
ZGBOT.loop.create_task(digitalpicloop())
ZGBOT.loop.create_task(bloom_pfploop())
ZGBOT.loop.create_task(autoname_loop())
ZGBOT.loop.create_task(autobio_loop())
ZGBOT.loop.create_task(custompfploop())
