...
from userbot import BOTLOG_CHATID, ZGBOT
...
@ZGBOT.zazbhai_cmd(
    ...
    info={
        ...
        "var name": "**[list of vars]**(https://ZGPRO.gitbook.io/ZGPRO/data-vars-setup)",
        "usage": [
    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                USERINFO = await ZGBOT.get_entity(ZGBOT.uid)
                FULL_USERINFO = (await ZGBOT(GetFullUserRequest(ZGBOT.uid))).full_user
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await ZGBOT.get_profile_photos(ZGBOT.uid)
                    myphoto = await ZGBOT.download_media(photos[0])
...
@ZGBOT.zazbhai_cmd(
    ...
    info={
        "header": "To customize your ZGPRO.",
        "options": {
...
async def custom_ZGPRO(event):
    "To customize your ZGPRO."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
...
@ZGBOT.zazbhai_cmd(
    ...
    info={
        "header": "To delete costomization of your ZGPRO.",
        "options": {
...
async def custom_ZGPRO(event):
    "To delete costomization of your ZGPRO."
    input_str = event.pattern_match.group(1)
    if input_str == "pmpermit":
