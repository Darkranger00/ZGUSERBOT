...
async def _(event):
    ...
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of ZGBOT are :"
    )
...
async def _(event):
    ...
    if found := [i for i in sorted(list(CMD_INFO)) if cmd in i]:
        ...
        out = f"I can't find any such command `{cmd}` in ZGBOT"
