...
from . import BOTLOG, BOTLOG_CHATID, LyricsGen, ZGBOT
...
# [----------------------------------------------------------------------]
# Errors
no_sp_vars = "Vars `SPOTIFY_CLIENT_ID` & `SPOTIFY_CLIENT_SECRET` are missing, add them first !\n\n[Follow this tutorial](https://telegra.ph/Steps-of-setting-Spotify-Vars-in-ZGBOT-04-24-2)"
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await ZGBOT.send_message(BOTLOG_CHATID, string)
            ...
            # 204 means user plays nothing
                ...
                if save_spam("spotify", False):
                    ...
                    await ZGBOT.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await ZGBOT(GetFullUserRequest(ZGBOT.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if new_bio != bio:
                            try:
                                await ZGBOT(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio and save_spam("telegram", True):
                        ...
                        await ZGBOT.send_message(BOTLOG_CHATID, to_send)
                else:
                    if save_spam("telegram", False):
                        stringy = (
                    ...
                    if not bio or "🎶" not in bio and bio != old_bio:
                        ...
                        await ZGBOT(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await ZGBOT(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
        ...
        USER_INITIAL_BIO["bio"] = (
            (await ZGBOT(GetFullUserRequest(ZGBOT.uid))).full_user
        ).about or ""
...
def file_check():
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/zazbhaianand2/ZG-V2-Resources/raw/master/Resources/Spotify/cat.png",
            logo,
        )
    if not os.path.exists(font_mid):
        urllib.request.urlretrieve(
            "https://github.com/zazbhaianand2/ZG-V2-Resources/blob/master/Resources/Spotify/GoogleSans-Medium.ttf?raw=true",
            font_mid,
        )
    if not os.path.exists(font_bold):
        urllib.request.urlretrieve(
            "https://github.com/zazbhaianand2/ZG-V2-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            font_bold,
        )
...
async def make_thumb(url, client, song, artist, now, full):
    ...
    try:
        ...
        myphoto = urllib.request.urlretrieve(
            "https://github.com/zazbhaianand2/ZG-V2-Resources/raw/master/Resources/Spotify/SwagZG.jpg"
        )
...
async def spotify_now(event):  # sourcery skip: remove-duplicate-dict-key
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await ZGBOT(unblock("ZGMusicRobot"))
            purgeflag = await conv.send_message(link)
