...
def get_thumb(name):
    url = f"https://github.com/rishabhanand2/ZG-V2-Resources/blob/master/Resources/Inline/{name}?raw=true"
...
async def article_builder(event, method):
    ...
    title = "ZG Userbot"
    description = "Button menu for ZGBOT"
    if method == "help":
        help_info = main_menu()
        title = "Help Menu"
        description = "Help menu for ZGBOT."
    ...
    if method == "ls":
        ...
        media = "https://github.com/rishabhanand2/ZG-V2-Resources/raw/master/Resources/Inline/filemanager.jpg"
    elif method == "deploy":
        media = "https://github.com/rishabhanand2/ZG-V2-Resources/raw/master/Resources/Inline/catlogo.png"
        title = "ᴛʜᴀɴᴏꜱ-ᴠᄅ"
        description = "Deploy yourself."
        query = "𝗗𝗲𝗽𝗹𝗼𝘆 𝘆𝗼𝘂𝗿 𝗼𝘄𝗻 ʑɠ ცơɬ."
        buttons = [
            (
                Button.url("Source code", "https://github.com/Zazbhai/ZGUSERBOT"),
                Button.url("Deploy", "https://github.com/Zazbhai/ZGUSERBOT"),
            )
        ]
        ...
        buttons = [
            (
                ...
                Button.url("Repo", "https://github.com/Zazbhai/ZGUSERBOT"),
            )
        ]
        try:
        ...
        description = "Alive menu for ZGBOT."
        ALIVE_PIC = gvarstatus("ALIVE_PIC")
        IALIVE_PIC = gvarstatus("IALIVE_PIC")
        if IALIVE_PIC:
            ZGS = [x for x in IALIVE_PIC.split()]
            PIC = list(ZGS)
            media = random.choice(PIC)
        if not IALIVE_PIC and ALIVE_PIC:
            ZGS = [x for x in ALIVE_PIC.split()]
            PIC = list(ZGS)
            media = random.choice(PIC)
        ...
        try:
            ...
            media = "https://github.com/rishabhanand2/ZG-V2-Resources/raw/master/Resources/Inline/spotify_off.png"
            if (
                ...
                buttons = [
                    Button.url(
                        "Tutorial",
                        "https://telegra.ph/Steps-of-setting-Spotify-Vars-in-ZGBOT-04-24-2",
                    )
                ]
            else:
                response = sp_data(
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    category = str(event.pattern_match.group(1).decode("UTF-8"))
    buttons = paginate_help(0, GRP_INFO[category], category)
    text = f"**ZGegory: **{category}\
...
async def on_plug_in_callback_query_handler(event):
    ...
    if mtype == "plugin":
        ...
        text = f"**Plugin: **`{category}`\
                \n**ZGegory: **__{getkey(category)}__\
                \n**Total Commands:** __{len(PLG_INFO[category])}__"
    await event.edit(text, buttons=buttons)


@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
    ...
    if found := [i for i in sorted(list(PLG_INFO.keys())) if query in i]:
        for plugin in found:
            ...
            if count > 1:
                ...
                result = builder.article(
                    title=title,
                    description=f"ZGegory:  {getkey(plugin)}\nTotal Cmd: {count}",
