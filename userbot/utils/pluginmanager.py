...
def remove_plugin(shortname):
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(ZGBOT._event_builders))):
            ev, cb = ZGBOT._event_builders[i]
            if cb.__module__ == name:
                del ZGBOT._event_builders[i]
