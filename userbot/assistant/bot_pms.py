...
from userbot import Config, ZGBOT
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"Hey! 👤{mention},\
                        ...
                        \n\nPowered by [ZGBOT](https://t.me/zgbotsupport)"
        buttons = [
            (
                Button.url("Repo", "https://github.com/Zazbhai/ZGUSERBOT"),
                Button.url(
                    "Deploy",
                    "https://github.com/Zazbhai/ZGUSERBOT",
                ),
            )
        ]
    else:
...
@ZGBOT.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@ZGBOT.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await ZGBOT.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await ZGBOT.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await ZGBOT.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await ZGBOT.tgbot.get_entity(BOTLOG_CHATID)
            await ZGBOT.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await ZGBOT.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await ZGBOT.get_entity(user_id)
    except Exception as e:
...
@ZGBOT.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@ZGBOT.bot_cmd(incoming=True, func=lambda e: e.is_private)
@ZGBOT.bot_cmd(edited=True, func=lambda e: e.is_private)
